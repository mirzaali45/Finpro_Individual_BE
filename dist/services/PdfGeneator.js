...
const generateInvoicePdf = (invoice) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const formatCurrencyForAmountPaid = (amount) => {
                const numAmount = typeof amount === "string" ? parseFloat(amount) : amount;
                return `Rp ${numAmount}`;
            };
            // Warna tema
            ...
            doc
                .fontSize(12)
                .fillColor(simplifiedBalanceDue <= 0 ? "#27ae60" : "#e74c3c")
                .text(formatCurrency(simplifiedBalanceDue), summaryX + 160, balanceDueY, {
                width: 90,
                align: "right",
                lineBreak: false,
            });
            ...
            if (invoice.payments && invoice.payments.length > 0) {
                ...
                invoice.payments.forEach((payment, index) => {
                    ...
                    doc
                        ...
                        .text(payment.reference || "-", paymentColX.reference + 5, paymentYPos - 5)
                        .fillColor("#27ae60")
